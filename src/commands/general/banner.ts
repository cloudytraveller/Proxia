...
export class BannerCommand extends ProxiaCommand {
    ...
    if (!member.bannerURL()) {
      await interaction.followUp({
        embeds: [
          {
            title: interaction.getString("global.ERROR"),
            description: interaction.getString("general.COMMAND_BANNER_EMPTY", {
              username: member.tag,
            }),
            color: this.bot.config.colours.error,
          },
      ...
      embeds: [
        {
          ...
          author: {
            name: interaction.getString("general.COMMAND_BANNER_DESCRIPTION", {
              username: member.tag,
            }),
