...
/**
 * Loads all Proxia commands
 * @param bot An instance of the Proxia client to push commands to
 * @param directory The directory to look for commands in
 */

export async function loadCommands(bot: ProxiaClient, directory: PathLike): Promise<any> {
  ...
  for (const file of files) {
    ...
    let commandToLoad: CallableProxiaCommand;
...
export async function loadEvents(bot: ProxiaClient, directory: PathLike, isLogger = false) {
  ...
  for (const file of files) {
    ...
    let eventToLoad: CallableProxiaEvent;
    ...
    // Creates the event
    const fileName = file.name.split(moduleFiletypeRegex)[0];
    const event = new eventToLoad(bot, fileName) as ProxiaEvent | ProxiaLogger;
