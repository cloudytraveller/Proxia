...
export class ProxiaLocaleSystem {
  ...
  constructor(path: PathLike, defaultLocale: ProxiaLocaleCode) {
  ...
  public getLocale(
    language: ProxiaLocaleCode,
    fieldName: ProxiaLocaleStrings,
    args?: { [x: string]: any },
  ): string {
    ...
    if (args) {
      ...
      Object.getOwnPropertyNames(args).forEach((argument) => {
        ...
        if (isOptional)
          output = output.replace(isOptional[1], args[argument] === undefined ? "" : isOptional[2]);
        ...
        if (plurals) {
          // Plural option
          let plural = "";
          if (args[argument] === 1 || (args[argument] > 1 && args[argument] < 2))
            plural = plurals[2];
  ...
  public getLocaleFunction(language: ProxiaLocaleCode): getString {
    return (fieldName: ProxiaLocaleStrings, args?: Record<string, any>) =>
      this.getLocale(language, fieldName, args);
  }
  ...
  public async getUserLocale(user: DiscordSnowflake, bot: ProxiaClient) {
    const userConfig = await bot.db.getUser(user);
  ...
  private _findLocaleString(
    ...
    if (!this.locales?.[language]?.[category[0]] && !this.locales?.[fieldName]) {
      ...
      this.locales?.[language]?.[category[0]] &&
      this.locales?.[language]?.[category[0]]?.[category[1]]
    ) {
